...
Text.print = function(...)
	...
	if type == 2 then
        for c = 1, #string do
			local fimg = Graphics.sprites.other['Font2_1'].img
			local simg = Graphics.sprites.other['Font2S'].img
			
            if(c >= 48 and c <= 57) then
                C = (c - 48) * 16
                Graphics.drawImage(fimg, x + B, y, C, 0, 15, 17)
                B = B + 16
            ...
            elseif(c >= 58 and c <= 64) then
                C = (c - 58 + 15) * 16
                Graphics.drawImage(simg, x + B, y, C, 0, 15, 17)
                B = B + 16
